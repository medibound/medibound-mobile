...
exports.unwrapDeviceKey = functions.region('us-central1').runWith({
		memory: '128MB'
  }).https.onCall(
  async (data, context) => {
    if (!context.auth.uid) {
    ...
    if (!publicWrappedKey) {
      throw new functions.https.HttpsError('invalid-argument', 'Public Wrapped key is required');
    }
    
    const privateWrappedKey = data.privateWrappedKey;
    if (!privateWrappedKey) {
      throw new functions.https.HttpsError('invalid-argument', 'Private Wrapped key is required');
    }
    ...
    try {
      ...
      throw new functions.https.HttpsError('internal', 'Failed to unwrap device key');
    }
  }
);